#!/usr/bin/env python3
"""
ChicoIA Bot - Versão para TODOS os usuários (24/7)
Este bot atende QUALQUER pessoa que mandar mensagem no @ChicoIA_bot
"""
...
_Tô aqui pra te ajudar quando precisar!_ 🎯"""

    elif 'flamengo' in text.lower() or 'palmeiras' in text.lower() or 'vs' in text.lower() or 'x' in text.lower():
        response = f"""⚽ *Análise de Jogo*

Olá, {user_name}! Vejo que você quer analisar um jogo específico.

*Para uma análise completa, me informe:*

1️⃣ *Times:* Quem joga contra quem?
2️⃣ *Campeonato:* Qual competição?
3️⃣ *Data:* Quando é o jogo?

*Exemplo:*
"Quero analisar Flamengo x Palmeiras, Brasileirão, domingo"

*Enquanto isso, algumas dicas gerais:*
• Analise o momento dos times
• Veja confrontos diretos recentes
• Considere desfalques importantes
• Não aposte mais que 3-5% da banca

_Aguardo mais detalhes para te ajudar melhor!_ 📊"""

    elif 'ajuda' in text.lower() or 'help' in text.lower():
        response = f"""Opa, {user_name}! Precisa de ajuda? 👋

Use o comando /ajuda para ver todos os comandos disponíveis.

Ou me pergunte diretamente:
...
def main():
    """Função principal - Bot rodando 24/7"""
    ...
    while True:
        try:
            # Buscar mensagens (long polling - aguarda até 30s por novas mensagens)
            ...
            print(f"   • Tempo de execução: {datetime.now().strftime('%d/%m/%Y às %H:%M:%S')}")
            print()
            print("=" * 70)
